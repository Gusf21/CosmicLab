...
window.addEventListener("beforeunload", () => {
    fetch(`https://cosmiclabapi.co.uk:2030/api/Data/DeleteSimulation?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}`, {
        method: "POST",
        headers: {
...
async function PassSystemToBackend() {
    ...
    const response = await fetch(`https://cosmiclabapi.co.uk:2030/api/Data/StartSimulation`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        ...
        frames = await fetch(`https://cosmiclabapi.co.uk:2030/api/Data/GetFrames?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}&timescale=3600&num=2000`);
    });

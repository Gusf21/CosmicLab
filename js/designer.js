    ...
    if (valid) {
        if (state == 0) {
            ...
            if (!adding) {
                ...
                await fetch(`http://94.4.213.191:2030/api/Data/EditObject?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}`, {
                    method: "POST",
                    headers: {
                ...
                await fetch(`http://94.4.213.191:2030/api/Data/CreateObject?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
            ...
            if (!adding) {
                ...
                await fetch(`http://94.4.213.191:2030/api/Data/EditOrbit?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
                ...
                await fetch(`http://94.4.213.191:2030/api/Data/CreateOrbit?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
...
window.Delete = async (element) => {
    ...
    if (state == 0) {
        const response = await fetch(`http://94.4.213.191:2030/api/Data/DeleteObject?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}&objectId=${id}`, {
            method: "POST"
        });
        Clear();
        LoadData("objects");
        ...
        const response = await fetch(`http://94.4.213.191:2030/api/Data/DeleteOrbit?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}&orbitId=${id}`, {
            method: "POST"
        });
        Clear();
        LoadData("orbits");
...
async function LoadData(type) {
    const response = await fetch(`http://94.4.213.191:2030/api/Data/GetUserCreations?sessionId=${GetCookie("sessionId").replace(/['"]+/g, '').toUpperCase()}`);
